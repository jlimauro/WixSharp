...
namespace Microsoft.Deployment.Samples.EmbeddedUI
{
    ...
    public class SampleEmbeddedUI : IEmbeddedUI
    {
        ...
        public bool Initialize(Session session, string resourcePath, ref InstallUIOptions internalUILevel)
        {
            ...
            int waitResult = WaitHandle.WaitAny(new[] { this.installStartEvent, this.installExitEvent });
            if (waitResult == 1)
            {
        ...
        public void Shutdown()
        {
            ...
            InUIThread(setupWizard.EnableExit);
            this.appThread.Join();
        }
        ...
        void Run()
        {
            using (setupWizard = new SetupWizard(installStartEvent))
